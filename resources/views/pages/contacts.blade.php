...
@include('partials.breadcrumbs',['template' => 'UnionRule', 'title'=> $page->title])

<section class="connection">
    <div class="container">
        <form id="reqFeedback" action="{{ route('feedback') }}" method="POST">
            @csrf
            <p class="text">@lang('general.thankYouForYourInterest')</p>
            <div class="radio">
                <div>
                    <input type="radio" id="choice1" name="check" value="choice1" checked>
                    <label for="choice1">@lang('general.joinTheUnion')</label>
                </div>

                <div>
                    <input type="radio" id="choice2" name="check" value="choice2">
                    <label for="choice2">@lang('general.btherBusinessInquiries')</label>
                </div>

                <div>
                    <input type="radio" id="choice3" name="check" value="choice3">
                    <label for="choice3">@lang('general.reportTechnicalProblemOnTheSite')</label>
                </div>
            </div>
            <div class="input">
